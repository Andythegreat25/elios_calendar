...
export function SettingsModal({ isOpen, onClose, profile, onSave }: SettingsModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      await onSave({ displayName, color, photoURL });
      onClose();
    } catch (error) {
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="flex items-center gap-4">
            {photoURL ? (
              <img src={photoURL} alt="Preview" className="w-16 h-16 rounded-full object-cover border border-zinc-200" referrerPolicy="no-referrer" />
            ) : (
              <div className="w-16 h-16 rounded-full bg-zinc-100 border border-zinc-200 flex items-center justify-center text-zinc-400 font-medium text-xl">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              />
              <span className="text-sm text-zinc-500 uppercase">{color}</span>
            </div>
          </div>
          ...
          <div className="pt-2">
            <button
              type="submit"
              disabled={isSaving}
              className="w-full bg-zinc-900 hover:bg-zinc-800 text-white px-4 py-3 rounded-xl font-medium transition-all shadow-sm disabled:opacity-50"
            >
