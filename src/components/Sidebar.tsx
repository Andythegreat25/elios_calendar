...
import { Plus, Calendar as CalendarIcon, Users, Building2, ChevronLeft, ChevronRight, Bell, Clock, LogOut, Settings } from 'lucide-react';
        ...
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-3">
            {profile?.photoURL || user?.photoURL ? (
              <img src={profile?.photoURL || user?.photoURL} alt={profile?.displayName || user?.displayName || ''} className="w-10 h-10 rounded-full border border-white/10 object-cover" referrerPolicy="no-referrer" />
            ) : (
            ...
            <div className="overflow-hidden">
              <div className="text-sm font-medium text-white truncate">{profile?.displayName || user?.displayName}</div>
              <div className="text-xs text-zinc-400 truncate">{user?.email}</div>
            </div>
          </div>
          <div className="flex items-center gap-1">
            <button onClick={onOpenSettings} className="w-8 h-8 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center transition-colors flex-shrink-0" title="Impostazioni">
